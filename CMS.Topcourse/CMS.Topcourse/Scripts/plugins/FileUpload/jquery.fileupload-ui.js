    ...
    $.widget('blueimpUI.fileupload', parentWidget, {
        ...
        options: {
            ...
            add: function (e, data) {
                ...
                //        files[i].sizeImg = sizeUpload[i];
                //    }
                //}
                
                $(this).fileupload('resize', data).done(data, function () {
                    
                    that.isValidated = data.files.valid = data.isValidated = that._validate(files);
                    that.context = data.context = that._renderUpload(files)
            ...
            send: function (e, data) {
                debugger
                var that = $(this).data('fileupload');
                if (!data.isValidated && !that.isValidated) {
                    if (!data.isAdjusted) {
        ...
        _hasError: function (file) {
            ...
            if (file.sizeImg && this.options.imageSize)
            {
                var checkSize = false;
                $.each(this.options.imageSize, function (index, value) {
                    if (file.sizeImg.width == value[0] && file.sizeImg.height == value[1]) {
                        checkSize = true;
                    }
                });
                if (checkSize == false)
